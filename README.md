...
> **Costs money to deploy and run.** Agent Echelon runs entirely in *your* AWS account, so you pay for the AWS resources it provisions and for every model invocation it makes. Deploying it, running the demo/validation suite (which drives real, billed conversations - see [End-to-End Tests](#end-to-end-tests)), and serving real traffic all incur real charges - Amazon Bedrock inference, the always-on infrastructure baseline, and any paid external providers (e.g. OpenAI/FAL image generation) you configure. **Every cost figure in this documentation is a rough estimate for orientation only** - your actual cost depends on region, traffic, retention, and configuration, and AWS pricing changes. Validate against the [AWS Pricing Calculator](https://calculator.aws) and your own bills before committing to any spend. A built-in, opt-in "sleep mode" (`-c sleepMode=true`, Aurora mode only) can auto-pause the Aurora cluster to 0 ACU after a configurable idle period (with a manual admin wake), shedding the dominant idle cost when nobody is using the deployment (see [SPEC-COST-SLEEP-MODE](docs/specs/ops/SPEC-COST-SLEEP-MODE.md)). Even so, this is a full multi-service governed platform on real infrastructure, so expect it to cost more to run than a typical single-purpose demo.
...
- **Conversation Archive, Leave & Remove** - A moderator can archive a conversation (read-only + hidden from the active list; membership retained, so members keep read-only access until the 90-day expiry) or remove a member; any member can leave (see [SPEC-CONVERSATION-ARCHIVE-AND-MEMBERSHIP](docs/specs/interaction/conversation/SPEC-CONVERSATION-ARCHIVE-AND-MEMBERSHIP.md))
