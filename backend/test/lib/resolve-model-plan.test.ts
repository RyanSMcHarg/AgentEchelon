 ...
 * return EXACTLY today's resolution — experiment-override-else-intent/tier — as a Bedrock plan.
...
describe('resolveModelPlan — new signals are inert when routing is disabled', () => {
  it('a CN segment + consent does NOT change the plan when deps lack enabled/cnProvider', () => {
    const ctx = { tier: 'standard' as const, intent: 'general', segment: { country: 'CN' }, externalModelConsent: true };
    const withSeg = resolveModelPlan(ctx, deps); // deps has no `enabled`/`cnProvider`
    const without = resolveModelPlan({ tier: 'standard', intent: 'general' }, deps);
...
describe('resolveModelPlan — CN routing rule (enabled + external provider)', () => {
  ...
  it('zh user + consent default → routes to the external CN provider, Bedrock as fallback', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'zh' }, cnDeps);
  ...
  it('CN request segment (even with en) → routes to the external CN provider', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'en', segment: { country: 'CN' } }, cnDeps);
    expect(plan.ref.provider).toBe('deepseek');
  });

  it('explicit consent=false blocks the external provider (stays Bedrock)', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'zh', externalModelConsent: false }, cnDeps);
    expect(plan.ref.provider).toBe('bedrock');
  });

  it('en + non-CN segment stays on Bedrock', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'en' }, cnDeps);
    expect(plan.ref.provider).toBe('bedrock');
  });

  it('disabled flag keeps a zh turn on Bedrock', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'zh' }, { ...cnDeps, enabled: false });
    expect(plan.ref.provider).toBe('bedrock');
  });

  it('consent default false (public phase) keeps a zh turn on Bedrock until the user opts in', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'general', userLanguage: 'zh' }, { ...cnDeps, externalConsentDefault: false });
    expect(plan.ref.provider).toBe('bedrock');
  });
});
...
describe('resolveModelPlan — DeepSeek-on-Bedrock CN routing (intent-routed, no consent gate)', () => {
  ...
  it('zh turn + reasoning intent → R1 (provider bedrock, reasoning=true)', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'report_generation', userLanguage: 'zh' }, cnDeps);
    expect(plan.ref.provider).toBe('bedrock');
  ...
  it('CN segment (even en) + non-reasoning intent → V3 (reasoning=false)', () => {
    const plan = resolveModelPlan(
      { tier: 'standard', intent: 'general', userLanguage: 'en', segment: { country: 'CN' } },
      cnDeps,
    );
  ...
  it('NO consent gate: zh turn routes to Bedrock DeepSeek even with externalModelConsent=false', () => {
    const plan = resolveModelPlan(
      { tier: 'standard', intent: 'report_generation', userLanguage: 'zh', externalModelConsent: false },
      cnDeps,
    );
    expect(plan.ref.modelId).toBe('us.deepseek.r1-v1:0');
  });
  ...
  it('Bedrock CN wins over a configured external provider', () => {
    const plan = resolveModelPlan(
      { tier: 'standard', intent: 'general', userLanguage: 'zh' },
  ...
  it('en + non-CN turn stays on the tier-default Bedrock model', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'report_generation', userLanguage: 'en' }, cnDeps);
  ...
  it('disabled flag keeps a zh reasoning turn on the default Bedrock model', () => {
    const plan = resolveModelPlan({ tier: 'standard', intent: 'report_generation', userLanguage: 'zh' }, { ...cnDeps, enabled: false });
    expect(plan.ref.modelId).not.toBe('us.deepseek.r1-v1:0');
  });
});
