/**
 * deriveClearance — the per-tier gate stamped onto KB chunks at ingestion.
 *
 ...
 */
import { deriveClearance } from '../../lambda/src/analytics-aurora/document-ingestion';

describe('deriveClearance (KB per-tier gate, fail-closed)', () => {
  afterEach(() => {
    delete process.env.RAG_DEFAULT_CLASSIFICATION;
  });

  it('uses an explicit tier segment: rag/{type}/{tier}/...', () => {
    expect(deriveClearance('rag/wiki/basic/onboarding.md')).toBe('basic');
    expect(deriveClearance('rag/wiki/standard/policy.md')).toBe('standard');
    expect(deriveClearance('rag/doc/premium/financials.md')).toBe('premium');
  });

  it('defaults untagged content to premium (most-restrictive, fail-closed)', () => {
    expect(deriveClearance('rag/wiki/onboarding.md')).toBe('premium');
    expect(deriveClearance('rag/doc/notes.md')).toBe('premium');
    expect(deriveClearance('rag/toplevel.md')).toBe('premium');
  });
  ...
  it('treats a non-tier second segment as untagged → default', () => {
    ...
    expect(deriveClearance('rag/wiki/handbook/page.md')).toBe('premium');
  });

  it('honors RAG_DEFAULT_CLASSIFICATION when set to a valid tier', () => {
    process.env.RAG_DEFAULT_CLASSIFICATION = 'basic';
    expect(deriveClearance('rag/wiki/onboarding.md')).toBe('basic');
    // an explicit segment still wins over the default
    expect(deriveClearance('rag/wiki/premium/secret.md')).toBe('premium');
  });
