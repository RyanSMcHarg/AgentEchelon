...
All three sit on **AWS managed primitives** (Amazon Chime SDK Messaging, Bedrock, S3, Cognito/STS, IAM, Kinesis), so inference, moderation, message delivery, and retention are AWS's to operate, not yours to build. The platform **integrates your identity provider** (Cognito by default, or your SSO/SAML/OIDC) rather than replacing it. Tools and knowledge are first-class: the assistant runs a **self-hosted tool loop** (not a managed agent service), reads tier-scoped context and RAG from S3, and reaches outside systems through pluggable connectors. It is not an MCP gateway today; the connector model fills that role, and governed MCP is a natural next step (see below).
