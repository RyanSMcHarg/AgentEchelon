...
- **`specs/communication/`** - the substrate that moves messages and keeps context (durable conversation, delivery, capture).
- **`specs/interaction/`** - who may act, as whom, at what capability (enforced in IAM). The five pillars: `identity-access/` (`core/` and `admin/`), `assistant-config/`, `conversation-config/`, `connectors/`, `auditing/`.
