...
- **Objective** - a small block that records *what decision the test informs* and *what would make you ship a variant*, so the experiment is a documented decision rather than a set of numbers that moved. It is advisory throughout: it frames the recommendation and never triggers a routing change on its own. It has four parts:
 ...
 - **Guardrails** (optional, up to three) - metrics that must not regress for the result to recommend a ship, each a metric plus a bound, for example "cost no worse than +25%" or "fallback rate no worse than +2%". A guardrail is a veto: a primary-metric win bought with a guardrail regression is not recommended as a ship, and the recommendation names the breached guardrail. Setting the decision rule up front this way also keeps the results honest (see Part 3).
...
- **Guardrails.** Each guardrail shows a pass or fail chip: a guardrail that has significantly regressed past its bound vetoes a ship recommendation even when the primary metric won, and the recommendation names it.
