...
Real user-to-assistant messages only - nothing is faked. Each turn is measured (TTFF, total latency, tokens, cost) and recorded to the archive/analytics, so the admin dashboard verification runs against genuine data. `npm run validate` runs ALL of the phases below in order (the admin phase runs after every data-producing phase on purpose); several are optional and self-skip when their deployed dependency is absent.
...
6. **Admin dashboard** - runs AFTER all of the above, so every tab is verified against the real data produced above. Further behavioral suites (auth, governance, latency, and more) follow it.
...
- Generate the per-package env files `frontend/packages/chat/.env` and `frontend/packages/admin/.env` from CDK outputs (`gen-frontend-env`) so `VITE_ANALYTICS_API_URL` / `VITE_EXPERIMENTS_API_URL` / `VITE_USER_FEEDBACK_API_URL` resolve (tasks / experiments / feedback). There is no root `frontend/.env`.
...
```bash
# Setup AND validation are one command. Its first three phases are the setup -
# self-knowledge, the platform-docs ingestion, then the demo seed - so a fresh
# deployment needs nothing run by hand beforehand.
cd backend
AWS_PROFILE=<your-profile> npm run validate
#    --skip-battle       skip the /battle phase (if not battle-enabled)
#    --only=seed         run one phase (e.g. seed, tasks, evaluate, admin). An
#                        invalid --only value makes the script print the full
#                        valid id set; the PHASES list in validate.mjs is the
#                        authority.

# Optional: point at a blog vault so the self-knowledge includes public posts. Only
# read by the `knowledge` phase, so set it on the same command.
AE_BLOG_VAULT_PATH="/path/to/your/blog-vault" AWS_PROFILE=<your-profile> npm run validate
```

> **The platform self-knowledge has two halves, and the run does both.** The CURATED half - a title plus
> a ~400-character summary per doc, read through the `load_platform_info` tool - is generated by the
> `knowledge` phase and uploaded by `seed-demo`. The RAG half - the full text of every doc, chunked and
> embedded into pgvector - is the `knowledge-rag` phase, and it is what answers "how does X work?" from
> the documentation instead of from summaries.
>
> `knowledge-rag` is **Aurora-only** and drops out of the phase list when no Aurora stack resolves
> (Athena mode has no vector store, and the curated half is the whole story there). It is also
> **optional**, so a transient failure warns and is named in the PHASE SUMMARY rather than stopping the
> run before `seed` - which would leave a deployment with no users and no context.
>
> Re-run it whenever the docs change (`npm run sync-knowledge:rag`, or `--only=knowledge-rag`). Skipping
> it is silent by nature: the assistant still answers platform questions, just from summaries, so the
> symptom is a thin answer rather than an error.

`npm run validate` is the orchestrator (`backend/scripts/validate.mjs`). It runs every phase in order - the setup phases (`knowledge -> knowledge-rag -> seed`) first, then the user flows and data-producing phases, then the admin dashboard phase after them (on purpose, so the dashboard has the real data to verify), then further behavioral suites (auth, governance, latency, and more) - and fails loudly on the first REQUIRED-phase failure. The `PHASES` list in `validate.mjs` is the authority on the current phase ids and their order. The data-producing phases (battle, image-gen, feedback, experiments, tasks, welcome, evaluate) are optional: each self-skips (or continues-with-a-warning) when its deployed dependency is absent. Watch the **DATA READINESS** preflight and the end-of-run **PHASE SUMMARY** to confirm the rich data actually landed - a phase can report `passed` yet have self-skipped.
