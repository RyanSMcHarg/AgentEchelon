...
**Status:** P0-P3 BUILT. P0-P1: the standalone admin app + the chat-app cutover. P2: the frontend is an npm-workspaces monorepo (`@ae/shared`, `@ae/chat`, `@ae/admin`) - shared code (types, apiCall with an injectable token provider, AuthProvider, the reused credential-exchange client, i18n) factored into `@ae/shared`, `chimeService` split into chat messaging (`@ae/chat`) + admin actions (`@ae/admin`) reusing the shared exchange, each app its own build/origin/env against the SAME instance/pool/users. P3: a dedicated admin Cognito app-client on the same pool (opt out with `-c adminAppClient=shared`, which falls back to the chat client). P4 (further docs/hardening) is the remainder. This document specifies extracting the admin console into its own independently-deployable frontend app on its own origin. The backend admin plane is already separable today (see Design Anchor); this spec completes the split on the frontend and deploy layers. Nothing here changes the admin authorization model.
...
**Configurable link-out (`VITE_ADMIN_APP_URL`).** Removing the embedded admin button (above) does not mean the chat app can never reference the console. When `VITE_ADMIN_APP_URL` is set, the chat Header shows an admin a LINK OUT to the console (a plain `<a href>` that opens the separate app; admin-only, opens in a new tab). This is a URL string, not admin code or an admin endpoint, so the chat bundle stays admin-free and `assert-no-admin-in-chat.mjs` still passes - the distinction the "remove the Admin button" change draws is between shipping operator *code* (forbidden) and pointing at a separately-served *URL* (allowed). `gen-frontend-env` emits it into the chat env from the admin frontend's `AdminDistributionUrl` output; a deployer can override it to point at their OWN admin surface built on the same admin APIs, so the operator interface stays pluggable. Unset - no link. The e2e admin specs target the same origin via `E2E_ADMIN_BASE_URL`.
