...
**Status:** Implemented (all phases). `lib/abuse-controls.ts` holds the logic, wired into all three tiers via `agent-classification-common.abuseControlsWiring`: request dedup (guard in `runSharedPipeline` + stable `correlationId` from `CHIME.message.id`), per-user/global spend budget with a fail-safe global ceiling, an SSM circuit trip, a per-tier hourly rate limit, and an inbound length cap. Active by default: dedup, rate limit (60/120/240 per tier per hour), and the length cap (16000 chars). Opt-in: the spend budgets (`-c bedrockUserHourlyBudget` / `-c bedrockGlobalHourlyBudget`) and, with a global budget, the circuit trip.
