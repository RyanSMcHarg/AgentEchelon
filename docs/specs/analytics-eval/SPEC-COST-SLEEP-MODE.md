...
**Status:** Implemented (Aurora-mode, opt-in `-c sleepMode=true`). Wired in `bin/backend.ts` (reads the `sleepMode` / `sleepAfterIdle` / `sleepCheckRate` / `sleepRecipients` context), provisioned by `analytics-stack-aurora.ts`, with the runtime in `lambda/src/deployment-sleep.ts` + `lib/sleep-mode.ts`.
**Scope:** A deploy-time opt-in that auto-pauses the Aurora data plane after a configurable idle period, and an admin/manual wake, to stop paying for an idle deployment. Aurora-mode only.

## Why

A shared dev account runs several instances of this platform. Cost is attributed per instance via the `Project` tag (see `docs/guides/admin/TAGGING.md`).
The idle cost of an Aurora-mode deployment is concentrated in a few always-on resources, and an idle instance keeps paying for them 24/7.
