...
- **Identity & Access** - `docs/specs/interaction/identity-access/core/SPEC-CREDENTIAL-EXCHANGE.md` (+ `docs/specs/interaction/identity-access/core/SPEC-FEDERATED-PARTICIPANTS.md`, `docs/guides/user/IDENTITY-PROVIDER-GUIDE.md`)
- **Assistant Configuration** - `docs/specs/interaction/assistant-config/SPEC-ASSISTANT-CONFIG.md`
- **Conversation Configuration** - `docs/specs/interaction/conversation-config/SPEC-CONVERSATION-TYPES.md`
- **Connectors** - `docs/specs/interaction/connectors/SPEC-CONNECTORS.md` (+ `docs/specs/interaction/conversation-config/SPEC-CONVERSATION-TYPES.md` §6, `backend/lib/config/connectors.ts`)
- **Auditing** - `docs/specs/interaction/auditing/SPEC-ACCESS-AND-CONTROLS-AUDITING.md`

Also: `docs/guides/developer/MESSAGE-FLOW.md` (how a turn travels the layers: channel flow / Lex / `@all` / fulfillment / async processor, and where each control acts), `docs/overview/ARCHITECTURE.md` (how the shipped system works end-to-end today), and `docs/specs/interaction/identity-access/core/SPEC-CONVERSATION-SECURITY.md` (the security layers).
...
- **Connectors** - the platform **plugs into the business's existing systems** (Salesforce / ServiceNow / Jira / AWS Support / PagerDuty …) instead of replacing them: resolve people, sync records, fetch data, provide transports - per-vendor, per-tenant-isolated. → `connectors/SPEC-CONNECTORS.md` (summary: `SPEC-CONVERSATION-TYPES.md` §6).
