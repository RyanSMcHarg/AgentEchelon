...
**Coverage:** `e2e/access-auditing.spec.ts` - asserts the capture path is deployed AND its Kinesis subscription is Enabled (a disabled mapping stops the record with nothing erroring), then creates a real conversation and requires that specific channel to have a membership record readable through the admin view. The read is SigV4-signed, and asserts on the response body rather than the status, because the membership-history handler answers 200 with an archiveError field when the underlying read fails.

**Problem and who it's for:** Enforcement answers "is this allowed right now"; it does not answer the auditor's question - what happened, who could act on a sensitive conversation, and when did that access change. This is for security and compliance, and the admin/operator who must show that access is recorded and reviewable, not merely enforced - the team that would otherwise stand up its own audit pipeline off the raw event stream, or reach for a separate audit product. It turns the Amazon Chime SDK channel event stream (messages plus membership and moderator events, via Kinesis) into an append-only system of record, surfaced as the admin console's membership-history and access-review views, with control-plane changes in the same model.

**Site section:** Interaction layer, Auditing pillar.


The **Auditing** pillar of the interaction layer (`docs/specs/interaction/SPEC-INTERACTION-LAYER.md` is the map). The conversation event stream is the audit source: the capture path (Amazon Chime SDK channel events → Kinesis → archive) feeds the admin-console membership-history view, and the retention, query, and access contract over that archive governs how that record is kept and read. Control-plane access changes are part of the same audit model.

**Related:** `docs/specs/interaction/SPEC-INTERACTION-LAYER.md` (the model) · `docs/specs/interaction/identity-access/core/SPEC-MODERATION.md` (the three surfaces, incl. the near-real-time Kinesis tap) · `docs/specs/interaction/identity-access/core/SPEC-CREDENTIAL-EXCHANGE.md` (the identities and bearer-pinned roles being audited) · `docs/guides/admin/ADMIN-GUIDE.md` (the membership-history view) · `backend/lambda/src/admin-conversations.ts` (`membershipHistory`) · `backend/lambda/src/analytics-aurora/kinesis-archival.ts` · `backend/lambda/src/client-events.ts` · `backend/lib/stacks/analytics-stack.ts`.
...
- **Passive-access review (admin reads today; per-member reads a designed extension)** - admin reads are logged now (the credential-exchange scoped-vend audit plus per-request API access logs). Logging ordinary members' reads is a designed extension, not yet a default: members read direct-to-Amazon Chime SDK (`ListChannelMessages`/`GetChannelMessage`) rather than through a logged AE API, so covering who merely *viewed* a conversation depends on the mechanism in §4b. Where enabled it is attributable through the bearer-pin `sub`.
...
- **Privileged reads.** The credential exchange emits an `admin_scoped_credential_vend` record on every scoped admin vend. For moderation and other Amazon Chime SDK actions it names the human, the `${sub}-admin` identity, the channel, and the capabilities. Under IAM enforcement ([`DESIGN-ADMIN-ACTION-IAM-ENFORCEMENT.md`](../identity-access/admin/DESIGN-ADMIN-ACTION-IAM-ENFORCEMENT.md)) the same record covers the customer-message-content read (`view-messages`, A2): the exchange vends a short-lived `execute-api` credential for it and logs the vend with `plane:'archive'`, so "who could read this customer's conversation" is auditable next to "who could moderate it". The other archive/analytics reads ride the standing sign-on role, and their per-resource IAM grant is the record of who could reach each capability.

### 4b. Detection and proactive action (observe by default, act on opt-in)

Auditing is not only passive recording. The same event stream plus the classification/tier model let the platform **detect access that shouldn't be happening** and decide whether to act on it. Three signal families:
...
- **Disallowed actions** - attempts IAM denies (surfaced as `AccessDenied` in CloudTrail) or actions the conversation-level policy forbids for that role.
- **Passive reads** - a member who *reads* a conversation they shouldn't, not only one who posts. This is a **designed extension, not built**, because no shipped capture path sees an ordinary member's reads. A member reads direct-to-Amazon Chime SDK Messaging (`ListChannelMessages` / `GetChannelMessage`) using their own bearer-pinned `${sub}` credentials, so those reads are (1) **not in the channel event stream** - that stream carries messages plus membership and moderator events, not reads - and (2) **not seen by the API access logs**, since no AgentEchelon API Gateway sits in the member read path. The one AWS-native per-read mechanism would be **CloudTrail data events**, but Amazon Chime SDK Messaging is **not** a supported CloudTrail data-event resource type: the CloudTrail data-events resource-type table lists no `AWS::ChimeSDKMessaging::*` entry, so CloudTrail cannot attribute per-member reads. (Amazon Chime SDK control-plane calls appear in CloudTrail as *management* events; the high-volume messaging read/write operations are data-plane and are not captured.) Covering who merely *viewed* a conversation therefore requires routing reads through a **logged backend read-proxy** - an AgentEchelon API that authorizes the caller, records their `sub`, and forwards to Chime - which is a major change to today's direct-to-Amazon Chime SDK read path (added latency on every message fetch, plus per-read log volume and cost). Where such a proxy is stood up, the bearer-pin `sub` makes each read attributable, and a deployer may scope logging to higher classifications or disable it to bound cost. Until then, passive-access coverage is limited to **admin reads** (the credential-exchange scoped-vend audit and the admin API access logs), not ordinary-member reads.

**Detection runs on three triggers, so a silent member can't hide:** (1) the membership and send events as they stream - the Kinesis consumer filters `CREATE`/`UPDATE_CHANNEL_MEMBERSHIP` (`membership-audit.ts`, `AUDITED_EVENT_TYPES`); (2) a **scheduled sweep** of every conversation's memberships against the current channel `classification` and each member's tier - the catch-all for members no live event would surface, driven by an EventBridge rule that invokes the auditor with a `sweep` control event (`membership-audit.ts`, `sweepAllChannels`; `constructs/membership-audit.ts`, `MembershipSweepSchedule`, default `rate(6 hours)`, per-run channel cap); and (3) **immediately on a clearance downgrade** - `user-management.ts` (`syncClearanceGroup`) async-invokes the auditor with a `reeval-user` event when a user's new clearance ranks below one they held, re-checking that user's existing memberships at once (a Cognito-group change emits no Chime membership event, so the stream never re-checks them). All three funnel through one shared `evaluateMembership` check, and (toggle-on) remediate the memberships the member no longer qualifies for. This is the control-plane → data-plane link: changing a user's tier doesn't just affect future vends, it re-evaluates their existing memberships.

**Verified by:** `backend/test/membership-audit.test.ts` (the sweep flags an over-tier member the event stream missed; the on-downgrade re-eval flags a downgraded member still on a premium channel; the event-stream path and enforce/report-only behavior) and `backend/test/cdk-synth.test.ts` (the sweep EventBridge rule fires the auditor with `{type:'sweep'}`; the auditor's `ListChannels`/`ListChannelMembershipsForAppInstanceUser` grants; user-management's name-scoped `lambda:InvokeFunction` grant + `MEMBERSHIP_AUDIT_FN_NAME` env for trigger #3).
