...
**Coverage:** `e2e/profile-config.spec.ts`, `e2e/admin-profiles.spec.ts`
...
- **Built - per-deployment config seams:** the **persona** (`systemPrompt`) via `ASSISTANT_SYSTEM_PROMPT` / `-c assistantSystemPrompt`, and the **intent taxonomy** via `ASSISTANT_INTENT_PACK` / `-c assistantIntentPack` (`docs/specs/interaction/assistant-config/SPEC-CONFIGURABLE-INTENT-PACK.md`). Both let a deployment define *what its assistant is* without forking the agent loop. The **persona has since been folded into the versioned bundle** - it rides the profile version as a body in S3, editable with no deploy ([`SPEC-PORTABLE-PROFILES.md`](SPEC-PORTABLE-PROFILES.md), "Where it lives"), and this parameter is now its fallback rather than its home. The intent taxonomy is the seam still waiting for that move.
