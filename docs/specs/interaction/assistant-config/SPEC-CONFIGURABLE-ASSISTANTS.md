...
- **A state says who it awaits:** `awaits: { party: 'requester' }` is the declaration that moves ownership to the person and tells the pre-model decisions a continuation from a new subject ([`SPEC-TASK-STATE-TRANSITIONS.md`](../conversation/SPEC-TASK-STATE-TRANSITIONS.md) sections 4 and 12.6). `requester` is the only reference that ships. A version stored with the deprecated `awaitsUser: true` is accepted and normalized to the same reference, so an existing profile version still validates and activates; a version naming a party nothing resolves is refused at the write path.
