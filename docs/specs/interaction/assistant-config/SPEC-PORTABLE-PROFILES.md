...
5. **Machines** - a per-assistant task machine is validated structurally, as the same checker the deployment pack uses applies (`validateTaskStateMachines`): every transition target is declared, a terminal carries a disposition, and a state that says who it awaits names a party the platform resolves. A state declares that with `awaits: { party: 'requester' }`, and the deprecated `awaitsUser: true` is accepted and normalized to the same reference, so a manifest exported before the declared form existed still imports and activates ([`SPEC-TASK-STATE-TRANSITIONS.md`](../conversation/SPEC-TASK-STATE-TRANSITIONS.md) section 12.6). A machine whose state awaits a party nothing resolves is rejected.
6. **Schema** - `schemaVersion` is compatible; older manifests upgrade through a documented migration. The guardrail change above did **not** bump it: the field's shape is unchanged and import accepts both forms (a key remaps, a provisioned id passes through), so a manifest exported before the change still imports correctly. Bumping would have rejected every earlier export for no gain in safety. The wait declaration did not bump it either, and for the same reason: both spellings are accepted, so no earlier export is invalidated.
