...
**To customize.** The synchronous gate is the `memberTier ≥ channelClassification` check in `share-conversation` and `create-conversation`; the containment is the tier tag gate; the audit backstop is the always-on membership-audit consumer, switched from report-only to auto-revoke with `-c membershipAuditEnforce=true` (`SPEC-CONVERSATION-SECURITY.md` §4b, Layer 6). This is the clearest case of control failing closed while the audit catches what the write-time gate cannot.
...
- [`SPEC-ADMIN-IDENTITY.md`](../admin/SPEC-ADMIN-IDENTITY.md): the two-identity admin model (chat `${sub}` vs `${sub}-admin`).
