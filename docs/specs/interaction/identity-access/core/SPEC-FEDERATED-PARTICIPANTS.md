...
Opt-in via `-c federatedUserPoolId`. Extends **Identity & Access** (`docs/specs/interaction/identity-access/core/SPEC-CREDENTIAL-EXCHANGE.md`) to humans who don't have an account in this deployment. Part of the interaction-layer set (`docs/specs/interaction/SPEC-INTERACTION-LAYER.md`).

**Related:** `docs/specs/interaction/identity-access/core/SPEC-CREDENTIAL-EXCHANGE.md` (the credential substrate this builds on) · `docs/specs/interaction/conversation-config/SPEC-CONVERSATION-TYPES.md` §6 (a connector resolves the external person) · `docs/guides/user/IDENTITY-PROVIDER-GUIDE.md` · `docs/specs/interaction/identity-access/core/SPEC-CONVERSATION-SECURITY.md` · AWS blog *"Integrate your Identity Provider with Amazon Chime SDK Messaging"* (June 2021).
