...
**Coverage:** `tests/e2e/credential-exchange.spec.ts`, `tests/e2e/signin.spec.ts`, `tests/e2e/classification-context.spec.ts`
...
| **Claim**               | `cognito:groups` in the JWT                             | The authoritative permission signal                             | `auth.ts` `parseGroups`/`extractClaims`/`CLASSIFICATION_ORDER`                                                              |
...
| KB retrieval - Aurora pgvector, **classification-filtered** | ✅¹ | ✅¹ | ✅¹ | SQL `metadata->>'classification' = ANY($3)` (`document-retrieval.ts`); router builds the classification scope |
...
- **KB classification filter.** Ingestion stamps `metadata.classification` via `deriveContentClassification` (from the `rag/{sourceType}/{classification}/` prefix; default `RAG_DEFAULT_CLASSIFICATION`, `premium` being the most-restrictive, fail-closed default), and retrieval filters on `metadata->>'classification' = ANY($3)` (`document-retrieval.ts`) with no `IS NULL` branch, so untagged chunks are not returned. Pinned by `test/lib/document-ingestion-classification.test.ts`. Aurora-mode only; the filter is classification-only (there is no finer-grained per-document `visibility` field).
...
- **Context-aware model routing runs on the standard tier only** (`contextRouting: true` in `standard-classification-stack.ts` alone); basic/premium use the tier-default model. **The configurable intent pack is provisioned for all classifications** (`intentPackParam: true` in all three profile stacks). The A/B experiment override, per-tier default model, and the separate Haiku classifier **are** wired on all tiers.
