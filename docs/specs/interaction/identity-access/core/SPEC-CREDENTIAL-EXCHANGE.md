...
| **admin (chat identity)** | admins | classification ceiling lifted to all tiers, still **membership-gated** and **bearer-pinned to own** identity | the above |

**Never on any end-user rung** (backend operations, not user credentials): `CreateChannel`, `CreateChannelMembership`, `CreateAppInstanceUser`, `UpdateChannelMessage`, `RedactChannelMessage`, `UpdateChannel`, `DeleteChannel` - channel creation / member admission / administration flow through the create-conversation, share, and admin paths (their own roles). Moderation (`RedactChannelMessage`, `DeleteChannelMessage`) and cross-channel admin reads are never a chat rung either; they are vended on the **admin plane** - a separate identity - described in §5a. The exchange selects the rung by the caller's **lifecycle state** (§6), not just their group.

### 5a. Vend planes and per-request capability scoping

A request names the **capabilities** it needs and, optionally, a single `channelArn`; the exchange vends a session policy scoped to exactly those actions on that resource, intersected with the rung role's ceiling. An STS session policy only ever narrows, so taking the capability set and channel scope from the request body is safe - a forged value can only reduce access. The default is the ordinary chat set (`view` + `participate`), so a client that sends no capabilities is unaffected. `view` + `participate` map to the read/send Amazon Chime SDK actions; `redact`, `delete`, `manage-membership`, `manage-channel`, and `rename` map to the mutating ones.

Every vend runs on one of two **identities**, never both in a request:

- **Chat plane (default)** - the caller's own `${sub}` on their clearance (or admin) rung. Never an `AppInstanceAdmin`, so the Amazon Chime SDK membership-gates every read; long-lived (`EXCHANGE_SESSION_SECONDS`, default 3600s); may be scoped to `channel/*`.
- **Admin plane** (`identity: "admin"`, admins only) - the caller's **separate** `${sub}-admin` identity, a standing `AppInstanceAdmin` provisioned here (idempotent). It is only ever vended a **channel-scoped, short-lived** (`EXCHANGE_MODERATION_SESSION_SECONDS`, default 1800s), **audited** credential, so cross-channel admin authority (an `AppInstanceAdmin` reads any channel without membership) never attaches to a broad chat credential. This is the two-identity split (`SPEC-ADMIN-IDENTITY.md`).

Moderation capabilities (`redact`, `delete`, `manage-membership`, `manage-channel`) are refused on the chat plane; they require `identity: "admin"` with a `channelArn`. On the admin plane the exchange serves three scoped sub-planes, each a different session policy and each exactly one request (never mixed):

- **Amazon Chime SDK moderation** - a `chime:*` session policy on the one named channel (redact / delete / membership / channel actions), bearer-pinned to `${sub}-admin`.
- **Archive read (A14)** - an `execute-api:Invoke` session policy on the admin conversations-messages API resource, so an operator reads a customer's stored message content short-lived and audited rather than from a standing role ([`DESIGN-ADMIN-ACTION-IAM-ENFORCEMENT.md`](../admin/DESIGN-ADMIN-ACTION-IAM-ENFORCEMENT.md)). The session policy pins the API resource, not the channel (IAM cannot condition `execute-api:Invoke` on a query parameter), so per-channel scope is enforced downstream.
- **Attachment read (S3)** - an `s3:GetObject` session policy on **only** the named channel's attachment keys. Two IAM-enforceable capabilities: `attachment-read` (assistant-generated deliverables under `generated-docs/<channelId>/*`, archive grade) and `attachment-read-uploads` (user-uploaded input under `attachments/<channelId>/*`, which may carry PII - moderation grade, flagged `sensitive` in the audit record). Because S3 ARNs condition on the object-key prefix, the credential is hard-bound to the one channel for its TTL. There is **no standing server-side S3 bearer**: the client presigns the `GetObject` with the vended credential. A future restricted admin role that omits a prefix from its ceiling is denied that prefix at the IAM layer, so the split is IAM-enforceable, not a code-only gate. The attachments bucket ARN reaches the exchange via SSM (a CDK prop would be circular).

Every admin-plane vend emits a structured audit record (`_auditEvent: admin_scoped_credential_vend`) naming the admin identity, the channel, the capabilities, and the TTL.
...
- **Session policies only narrow** - a per-request capability set or channel scope can reduce the rung ceiling but never widen it (STS session-policy semantics), so it is safe to take them from the request body. Moderation and attachment capabilities are additionally refused off the admin plane (§5a).
- **Admin authority is a separate identity** - cross-channel admin reads and moderation ride the standing `${sub}-admin` identity on channel-scoped, short-lived, audited credentials, so an admin's own chat identity is membership-gated like any user's and can never silently read a conversation it is not a member of (§5a, `SPEC-ADMIN-IDENTITY.md`).
...
- **Admin vend planes:** the admin-plane moderation, archive (`execute-api:Invoke`), and S3 attachment (`s3:GetObject`) session-policy vends live in the same exchange Lambda, each gated on `identity: "admin"` + a `channelArn` and each audited. The exchange unit tests cover all three planes (including that moderation and attachment capabilities are refused off the admin plane, and that an `attachment-read-uploads` vend is audited as `sensitive`).
