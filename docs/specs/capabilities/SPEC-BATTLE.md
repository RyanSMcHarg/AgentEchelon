...
**Status:** Implemented. Gated by the profile's `battleEligible` flag (`backend/lib/config/profiles.ts`); premium is the only eligible profile out of the box. **Layer:** Core platform (capability - a platform feature, not an interaction pillar; its MECHANISM lives here, its variant CONFIG is assistant-config, pillar 2) **Plane:** core **Technical design:** [DESIGN-BATTLE.md](./DESIGN-BATTLE.md)

**Coverage:** `e2e/battle.spec.ts` - FR1-FR9. FR10's answer-then-resume round trip is asserted by B-E7, which parks a side `WAITING_FOR_USER` via a task-shaped duel, answers it as the duel's owner, and asserts the side reaches `COMPLETED`. B-E8 asserts the below-premium capability refusal: a `/battle` in a channel whose classification is not battle-eligible is answered with an explanatory refusal and no duel starts.
...
- **As a channel moderator**, I open the members panel of a conversation at a battle-eligible classification (premium out of the box), pick an armed experiment, and turn on Battle Mode. The treatment assistant joins as a visible member with its display name, a system message announces it, and `/battle` becomes available. When I am done I turn it off and the assistant leaves cleanly.
...
**FR2 - Enable battle on a channel.** A moderator of a channel at a battle-eligible classification (premium out of the box) can turn Battle Mode on, choosing an armed experiment that matches the channel; the treatment variant joins as a real conversation member and a system message announces it. Turning it off removes the member and announces the departure. *Done when:* enabling adds a visible second assistant and makes `/battle` available; disabling removes it; a channel whose classification is not battle-eligible cannot enable battle.
...
How the composer routes an answer to the side that asked. The chat app derives the set of waiting sides from the channel's messages (each waiting side's placeholder carries a live waiting marker), so every member sees the same queue. The composer shows a "Replying to:" affordance naming the waiting assistant; when several sides are waiting it lists them all and defaults to the most recent waiter, and the user may deliberately address several at once. A reply resumes only the sides that are both explicitly addressed and currently waiting: a waiting side the user did not address stays waiting (it must not free-ride on a rival's clarification), and an addressed side that is not waiting is ignored. Code cites this section by name (`ConversationProvider.chime.tsx`, `channel-flow-processor.ts`, `battle-state.ts`).
