...
End-to-end (`tests/e2e/battle.spec.ts`): the full round-trip (two distinct sender ARNs on round 1, two round-2 placeholders, `NO_REBUTTAL` opt-out with no orphan placeholder, the not-enabled hint, the premium-only reply). Its behavioral cases are `test.fixme` and **deferred until a battle-enabled deploy** exercises them.
