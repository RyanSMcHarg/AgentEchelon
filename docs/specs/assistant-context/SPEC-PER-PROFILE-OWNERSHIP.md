...
> The per-profile assistant stacks are organized **profile-as-data**: one shared
...
> loop - there is **no Bedrock Agent** - so a profile stack owns the
...
construct references between a profile stack and the platform - so a profile deploys
on its own cadence.
...
action-group Lambda / Lex `BedrockAgentIntent`; the profile stack centers on the
processor.
...
Not present in a profile stack: `CfnAgent`, `CfnAgentAlias`, agent role, agent SSM
...
A profile stack **consumes** (SSM-resolved ARNs / plain props from the composition
...
Consumers of the profile contract: the **shared router** resolves
`/agent-echelon/assistant/{tier}/processor-arn` to dispatch; **`create-conversation`** resolves
`/agent-echelon/assistant/{tier}/bot-arn` to add the right profile bot to a new channel.
...
The profile stack resolves those ARNs (SSM at synth or plain props from the
...
How-to: see [`HOW-TO-ADD-OR-MANAGE-A-PROFILE.md`](../../guides/developer/HOW-TO-ADD-OR-MANAGE-A-PROFILE.md)
...
Because the cross-stack contract is SSM-only, each profile stack deploys on its own
...
to add the right profile bot to a new channel. A single-account demo still gets
...
- **Tier isolation** = the per-profile processor role's S3 scope. Keep
