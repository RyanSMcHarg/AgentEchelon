...
| `onboard_attendee` | `onboard_attendees` | delegates to platform | None direct - delegates to `capabilities/DESIGN-MULTI-AGENT-ORCHESTRATION.md` multi-agent orchestration, which supplies each greeter sub-agent's acting identity (the pooled alt-slot identities are the recommended source; vended-vs-pooled is an open question in `capabilities/DESIGN-MULTI-ASSISTANT-TURN-ENGINE.md`). |
...
- **Capability / IAM:** **None direct.** This tool hands the `{attendeeKey, briefing}` list to the platform multi-agent orchestration runtime, which owns spawning, per-sub-agent identity (classification-gated; the pooled alt-slot identities are the recommended source, and vended-vs-pooled is an open question in `capabilities/DESIGN-MULTI-ASSISTANT-TURN-ENGINE.md`), fan-out, and lifecycle. The meetings assistant holds no `chime:CreateAppInstanceBot` or bulk-invoke grant. See Part B.3.
...
|---|---|---|
| Multi-agent orchestration (`capabilities/DESIGN-MULTI-AGENT-ORCHESTRATION.md`) | Spawning one sub-agent per attendee, each sub-agent's identity (classification-gated; pooled alt-slot recommended, vended-vs-pooled open), the parallel fan-out, retries, and lifecycle/teardown. | Which attendees get a greeter, and the per-attendee briefing text each greeter delivers (the `greeters[]` it passes to `onboard_attendees`). |
| Credential-exchange (`interaction/identity-access/core/SPEC-CREDENTIAL-EXCHANGE.md`) | Vending short-lived, bearer-pinned credentials for a greeter IF the vended-identity option is chosen (the pooled alt-slot is the recommended source; vended-vs-pooled is an open question in `capabilities/DESIGN-MULTI-ASSISTANT-TURN-ENGINE.md`). | Nothing - it never handles a sub-agent's credentials. |
