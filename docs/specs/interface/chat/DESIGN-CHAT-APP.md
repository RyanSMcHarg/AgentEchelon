...
**Coverage:** `e2e/signin.spec.ts`, `e2e/signup.spec.ts`, `e2e/agent-intents.spec.ts`, `e2e/mentions.spec.ts`
...
Surface components (`packages/chat/src/components/`): `ConversationList`, `ConversationInterface` (message rendering, day dividers, `CollapsibleText`, empty-state greeting), `MessageInput` (composer, mentions, attachments, battle "Replying to"), `NewConversationModal` (classification and model pick), `ChannelMembersPanel` (roles, add/remove, Battle Mode toggle), `ShareConversationModal`, `ArchiveConversationModal`, `AttachmentDisplay`, `FileUploadPreview`, `ModelSelector`, `ConnectionStatus`, `DeploymentStatusBanner`, `TaskStatusIndicator`, `OpenWorkItems`, `BattleScorecard`, `BattleTallyBar`, and the auth screens `RegistrationScreen` and `EmailVerificationScreen` (login and forgot-password screens live in `@ae/shared`).
...
- **Open work items** (`OpenWorkItem[]` via `openWorkItems` / `refreshOpenWorkItems`): the work items the user owns across every conversation (`openWorkItemService`, GET `/tasks/mine`), oldest first - the cross-conversation half of "waiting on you" (`battleWaitingBots` derives from the loaded channel's messages, so it cannot show the item in a conversation the user is not looking at). Polled every 60 seconds and refreshed after a send (answering is what closes an item); the refresh keeps referential identity when nothing changed, so dependent effects do not re-fire on identical data.
- **Attachments**: uploaded to S3 via a presigned POST (signed fields; the POST policy's `content-length-range` is a server-enforced size cap); the app holds only the returned `Attachment` descriptor.
...
**Answering a work item, and its precedence over the sticky target.** When the user's next message answers an open work item in the active conversation, the composer shows an "Answering" affordance and routes the send to the assistant whose work it answers (with the item's task id), rather than to whoever last targeted the user: the answer target takes precedence over the sticky mention target, whose prefix stands down for that send (`MessageInput`). Without the precedence, the sticky prefix forced the message to the sticky human with no task id while the UI said "Answering", and the work item stayed blocked. After the send, `refreshOpenWorkItems` re-reads the queue, since answering is what closes an item.

**Attachments (FR-6).** `attachmentService.uploadFile` enforces the allowed-type list and 10 MB cap client-side, requests a Cognito-authorized presigned POST (signed fields), and uploads to S3 as a form POST whose policy enforces the size cap server-side (`content-length-range`), returning an `Attachment` descriptor sent as message metadata. Assistant attachment replies render via `AttachmentDisplay`.

**Battle chat surface (FR-9).** In a battle-enabled channel, `ConversationInterface` maps round-1 replies to `BattleScorecard` variants (label from the battle marker's resolved display name, response time and estimated cost from the compact per-message summary) and renders `BattleTallyBar` from `computeBattleTally`. `battleWaitingBots` is derived from messages still carrying a `battleWaiting` marker (insertion-ordered), driving the composer's "Replying to" affordance so a targeted reply routes to the waiting assistant. Enabling/disabling Battle Mode on a battle-eligible channel is the `ChannelMembersPanel` toggle calling `channelBattleService`.
...
- **Unit (Vitest, `packages/chat/src/`):** `components/CollapsibleText.test.tsx`, `components/Header.test.tsx`; `components/MessageInput.task-answer.test.tsx` (the answering affordance and its precedence over the sticky target), `components/OpenWorkItems.test.tsx` (the cross-conversation queue), `providers/ConversationProvider.sticky-target.test.tsx` (sticky-target pinning and clearing); `services/attachmentService.test.ts` (type/size validation and upload), `services/battleOutcomeService.test.ts`, `services/channelBattleService.test.ts`; `utils/mentionParser.test.ts`, `utils/battleTally.test.ts`, `utils/modelLabel.test.ts`. Shared parsing is covered by `packages/shared/src/utils/messageParser.test.ts` and `packages/shared/src/services/eventTrackingService.test.ts`.
- **End-to-end (Playwright, `tests/e2e/`):** `signup.spec.ts`, `signin.spec.ts` (custom auth screens), `welcome.spec.ts` (first-turn greeting), `onboarding-intake.spec.ts`, `mentions.spec.ts`, `classification-context.spec.ts` (classification-capped conversations), `agent-intents.spec.ts`, `tasks.spec.ts` and `task-state-machine.spec.ts` (task status), `feedback.spec.ts`, `battle.spec.ts` (the `/battle` chat flow and scorecard), `credential-exchange.spec.ts` (the chat-plane vend), `drift-detection.spec.ts` (the navigate-channel redirect).
